...
from .protocol import (SCHEMAS, MAX_FRAME, ProtocolError, ReplayCache, canonical,
                       decode, envelope, request, sign, verify)

LOG = logging.getLogger("butters_agent")


def verify_pin(ssl_object, expected):
    cert = x509.load_der_x509_certificate(ssl_object.getpeercert(binary_form=True))
    spki = cert.public_key().public_bytes(Encoding.DER, PublicFormat.SubjectPublicKeyInfo)
    if not __import__("hmac").compare_digest(hashlib.sha256(spki).hexdigest(), expected):
        raise ProtocolError("server_identity_mismatch")


class Client:
    def __init__(self, config, credentials, engine):
        self.config, self.credentials, self.engine = config, credentials, engine
    ...
    async def run(self):
        ...
        while True:
            try:
                ...
                code = str(exc) if isinstance(exc, ProtocolError) else "connection_unavailable"
                LOG.warning(json.dumps({"event": "disconnected", "reason": code}))
                if code in {"server_identity_mismatch", "unauthorized", "invalid_signature"}:
                    delay = 60
    ...
    async def session(self):
        url = self.config["url"]
        parsed = urlsplit(url)
        if (parsed.scheme != "wss" or parsed.path != "/agent/v1/session"
        ...
        async with connect(url, ssl=context, proxy=None, open_timeout=10,
                           max_size=MAX_FRAME, max_queue=8, ping_interval=15,
                           ping_timeout=20, close_timeout=3, compression=None,
                           user_agent_header=None) as ws:
            verify_pin(ws.transport.get_extra_info("ssl_object"), self.config["spki_sha256"])
            # No Authorization header: token is first sent AFTER pin verification.
            await ws.send(json.dumps({"type": "hello", "protocol": 1, "schema": 1,
                "agent_id": self.config["agent_id"], "version": AGENT_VERSION,
                "token": self.credentials["token"], "actions": sorted(SCHEMAS)}))
            ...
            await self._connected(ws, connection_id)

    async def _connected(self, ws, connection_id):
        active = {}
        async def send(kind, **fields):
            await ws.send(canonical(sign(envelope(kind, connection_id, **fields), self.key)).decode())

        async def heartbeat():
            seq = 0
            while True:
                state = await asyncio.to_thread(self.engine.platform.session)
                await send("heartbeat", seq=seq, session=state, version=AGENT_VERSION)
                seq += 1
        ...
        async def execute(frame, cancel):
            ...
            async def expire():
                await asyncio.sleep(max(0, frame["issued_at"] + frame["timeout_seconds"] - time.time()))
                cancel.set()
            ...
            try:
                ...
                if result is None:
                    result = await asyncio.to_thread(self.engine.invoke, frame["action"],
                                                     frame["parameters"], cancel)
                    self.cache.put(frame, result)
                await send("result", request_id=rid, result=result, duplicate=duplicate)
                LOG.info(json.dumps({"event": "result", "request_id": rid,
                    "action": frame["action"], "success": result["success"], "duplicate": duplicate}))
            finally:
                deadline.cancel()
                active.pop(rid, None)
            ...
            async for raw in ws:
                try:
                    ...
                    if frame.get("type") == "cancel":
                        if set(frame) != {"type", "connection_id", "issued_at", "sig", "request_id"}:
                    ...
                    existing = active.get(rid)
                    if existing:
                        if canonical(existing[0]) != canonical(frame):
                            raise ProtocolError("duplicate_request")
                        await send("ack", request_id=rid)
                        continue
                    # One live operation: no queued side effects and no racing idempotency keys.
                    if active:
                        raise ProtocolError("busy")
                    await send("ack", request_id=rid)
                    cancel = threading.Event()
                    active[rid] = (frame, cancel)
