"""Shared signed wire format; imported unchanged by both endpoints."""
...
SCHEMAS = {
    ...
    "desktop.vm.list": set(),
    "desktop.vm.status": {"vm"},
    "desktop.vm.start": {"vm"},
    "desktop.vm.stop": {"vm"},
}
MUTATIONS = frozenset({"desktop.app.launch", "desktop.vm.start", "desktop.vm.stop"})
...
def parameters(action, values):
    ...
    if any(not isinstance(v, str) or not NAME.fullmatch(v) for v in values.values()):
        raise ProtocolError("invalid_parameter")
    return dict(values)
...
def sign(frame, key):
    ...
    return {**unsigned, "sig": hmac.new(key, canonical(unsigned), hashlib.sha256).hexdigest()}
...
def verify(frame, key, connection_id, *, now=None):
    ...
    sig = frame.get("sig")
    if not isinstance(sig, str) or not hmac.compare_digest(sign(frame, key)["sig"], sig):
        raise ProtocolError("invalid_signature")
    return frame


def envelope(kind, connection_id, **fields):
    return {"type": kind, "connection_id": connection_id,
            "issued_at": time.time(), **fields}


def request(frame, *, target, now=None):
    allowed = {"type", "connection_id", "issued_at", "sig", "request_id", "action",
               "target", "parameters", "idempotency_key", "timeout_seconds"}
    ...
    for field in ("request_id", "idempotency_key"):
        try:
            if str(uuid.UUID(frame[field], version=4)) != frame[field]:
                raise ValueError()
...
class ReplayCache:
    ...
    def fingerprint(frame):
        return hashlib.sha256(canonical({k: frame[k] for k in
            ("action", "target", "parameters")})).hexdigest()
    ...
    def put(self, frame, result):
        for prefix, field in (("r", "request_id"), ("i", "idempotency_key")):
            self.entries[(prefix, frame[field])] = (
                self.clock(), self.fingerprint(frame), dict(result))
