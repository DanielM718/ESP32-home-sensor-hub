...
class Platform:
    ...
    def session(self):
        return {"state": "ACTIVE" if self.active else "NONE", "session_id": 1,
                "interactive_session": self.active, "gui_launch": self.active}

    def app_status(self, entry):
        return {"installed": entry["path"] not in self.missing,
                "running": entry["path"] in self.running,
                "session_id": 1, "visible_window": entry["path"] in self.running}
