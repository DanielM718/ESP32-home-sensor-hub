...
def powershell(script):
    ...
    result = subprocess.run(
        [PS, "-NoLogo", "-NoProfile", "-NonInteractive", "-EncodedCommand", encoded],
        capture_output=True,
        timeout=15,
...
def dpapi(data: bytes, *, protect: bool) -> bytes:
    """User scope only. Called in the user's logon context, never machine scope."""
...
class Platform:
    def __init__(self):
        ...
        if not self.kernel.ProcessIdToSessionId(
            os.getpid(), ctypes.byref(self.session_id)
        ):
    ...
    def session(self):
        ...
        if desktop:
            try:
                ...
                if self.user.GetUserObjectInformationW(
                    desktop, 2, name, ctypes.sizeof(name), ctypes.byref(length)
                ):
        ...
        if terminal.WTSEnumerateSessionsW(
            ...
            try:
                multiple = (
                    sum(
                        1
                        for item in sessions[: count.value]
                        if item.id != 0 and item.state in (0, 4)
                    )
                    > 1
                )
            finally:
        ...
        return {
            ...
            "gui_launch": interactive and unlocked and not multiple,
...
""")
        ...
        images = {os.path.normcase(path) for path in entry["images"]}
        matching = [
            process
            for process in processes
            if os.path.normcase(process["path"]) in images
            and process["session_id"] == self.session_id.value
        ]
        process_ids = {process["pid"] for process in matching}
        windows = []
    ...
    def launch(self, entry):
        ...
        process = subprocess.Popen(
            [entry["path"]],
            cwd=str(Path(entry["path"]).parent),
            shell=False,
        ...
        session_id = w.DWORD()
        if (
            self.kernel.ProcessIdToSessionId(process.pid, ctypes.byref(session_id))
            and session_id.value != self.session_id.value
        ):
