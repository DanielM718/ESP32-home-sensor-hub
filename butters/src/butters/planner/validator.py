...
class PlannerValidator:
    def __init__(
        ...
        *,
        max_actions: int = 3,
        compositions: frozenset[tuple[str, ...]] = frozenset(),
    ) -> None:
        if not 1 <= max_actions <= 4:
            raise ValueError("planner max_actions must be one to four")
        if any(len(item) > max_actions for item in compositions):
            raise ValueError("a reviewed composition exceeds planner max_actions")
        self.registry = registry
        self.max_actions = max_actions
        # Ordered action sequences a reviewed template has approved. A step
        # count below the limit is not permission to assemble a physical
        # sequence, so anything not listed here is refused.
    ...
    def validate(
        ...
        *,
        ...
        if len(steps) > 1:
            # Multi-stage physical behavior belongs to a registered workflow
            # action or an explicitly reviewed composition template. The planner
            # never earns the right to sequence actions on its own.
            sequence = tuple(step.action_id for step in steps)
            if sequence not in self.compositions:
                raise PlannerError(
                    "unsupported_composition",
                    "Multi-step plans require a reviewed composition template.",
                )
            if any(
                self.registry.get(step.action_id).action_class
                is not ActionClass.ACTION
                for step in steps
            ):
                raise PlannerError(
                    "unsupported_composition",
                    "A reviewed composition contains registered actions only.",
                )
        try:
            encoded = json.dumps(
