...
Use only the supplied recent conversation and approved READ_ONLY tools.
...
When tools are available, request at most one tool at a time. Distinguish observed data from hypotheses.
...
class OpenAIGeneralReasoner(GeneralCloudReasoner):
    ...
    def available(self) -> bool:
        return bool(
            ...
            and self.settings.allow_paid_calls
        )
    ...
    def reason(
        ...
        *,
        ...
        if not self.settings.enabled or not self.settings.allow_paid_calls:
            raise CloudReasonerError("cloud_disabled", "paid cloud reasoning is not enabled")
        if model not in self.settings.pricing:
            raise CloudReasonerError("model_denied", "model is not configured with reviewed pricing")
        if len(text.encode("utf-8")) > 8000:
            raise CloudReasonerError("context_too_large", "request exceeded the cloud context limit")
        ...
        if len(encoded_body) > 64 * 1024:
            raise CloudReasonerError("context_too_large", "cloud request context exceeded 65536 bytes")
        ...
        for attempt in range(self.settings.max_retries + 1):
            try:
                ...
                if attempt >= self.settings.max_retries or not (exc.code == 429 or exc.code >= 500):
                    raise CloudReasonerError("upstream_status", f"OpenAI returned HTTP {exc.code}") from exc
            except TimeoutError as exc:
                if attempt >= self.settings.max_retries:
                    raise CloudReasonerError("timeout", "OpenAI request timed out") from exc
        ...
        *,
        ...
        for item in output:
            ...
            if item.get("type") == "function_call":
                ...
                if not all(isinstance(value, str) for value in (name, call_id, raw_arguments)):
        ...
        if len(tool_requests) > 1 or (tool_requests and text_parts):
            raise CloudReasonerError("malformed_response", "response must contain one tool call or final text")
        ...
        if len(response_text) > 12000:
            raise CloudReasonerError("response_too_large", "model text exceeded the response limit")
        if not tool_requests and not response_text:
            raise CloudReasonerError("malformed_response", "response contains neither a tool call nor text")
