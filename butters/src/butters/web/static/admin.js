...
function renderTraces(traces){const container=document.querySelector("#trace-list");container.replaceChildren();for(const trace of traces){const card=document.createElement("details");card.className="trace-card";const summary=document.createElement("summary");summary.textContent=`${trace.source} · ${trace.trace_id} · ${trace.completed?"complete":"live"}`;const events=document.createElement("div");events.className="trace-events";for(const event of trace.events){const row=document.createElement("div");row.className="trace-event";const elapsed=document.createElement("code");elapsed.textContent=`${event.elapsed_ms}ms`;const stage=document.createElement("strong");stage.textContent=event.stage;const detail=document.createElement("span");detail.textContent=`${event.status}${event.reason_code?` · ${event.reason_code}`:""} ${JSON.stringify(event.fields)}`;row.append(elapsed,stage,detail);events.append(row);}card.append(summary,events);container.append(card);}}
