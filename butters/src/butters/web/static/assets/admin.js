...
async function waitForAdminAction(jobId){for(let attempt=0;attempt<30;attempt+=1){const job=(await api(`/api/actions/jobs/${encodeURIComponent(jobId)}`)).value;if(job.state==="completed")return job;if(["failed","cancelled","expired"].includes(job.state))throw new Error(job.failure_reason||"NAS wake did not complete");await new Promise(resolve=>window.setTimeout(resolve,500));}throw new Error("NAS wake is still running; check Actions / Broker for its final status");}
async function wakeNas(){const button=document.querySelector("#wake-nas");const status=document.querySelector("#wake-nas-status");button.disabled=true;button.textContent="Sending…";status.textContent="Submitting the fixed NAS wake action…";try{let result=(await api("/api/admin/tools/wake-nas",{method:"POST",body:JSON.stringify({})})).value;if(result.status==="authentication_required"){status.textContent="Passkey authentication required…";result=await authenticatePurpose("pending_action",null,result.pending_action.pending_action_id);}if(!Array.isArray(result.jobs)||!result.jobs.length)throw new Error("NAS wake was not queued");status.textContent="Wake action queued…";await waitForAdminAction(result.jobs[0].job_id);status.textContent="Wake packet sent.";}catch(error){status.textContent=`Wake NAS failed: ${error.message||"unknown error"}`;}finally{button.disabled=false;button.textContent="Wake NAS";}}
document.querySelector("#wake-nas").addEventListener("click",wakeNas);
