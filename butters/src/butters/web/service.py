...
from butters.assistant import AssistantResponse, DeterministicAssistant, create_assistant
...
class BetaAssistantService:
    def __init__(
        ...
        *,
        ...
        self.assistant = assistant or create_assistant(settings, vocabulary)
    ...
    def _handle_text_locked(
        ...
        *,
        ...
        current_trace.emit(
            TraceStage.REQUEST,
            "accepted",
            fields={"raw_text": text, "source": source, "admin_override": override.value if administrator else None},
        )
        ...
        features = self._complexity_features(normalized, route, diagnostic_request, override)
        current_trace.emit(
            TraceStage.ROUTING,
            route.status,
        ...
        if override is RouteOverride.LOCAL_DIAGNOSTIC:
            if diagnostic_request is None:
                result = self._unsupported_response(text, normalized, route, "diagnostic_playbook_not_matched")
                decision = RouteDecision("unsupported", ("diagnostic_playbook_not_matched",), features, override.value)
            else:
                result = self._execute_diagnostic(diagnostic_request, current_trace, force_cloud=False)
                decision = RouteDecision("diagnostic_local", ("diagnostic_playbook_match",), features, override.value)
        elif diagnostic_request is not None and override is not RouteOverride.DETERMINISTIC_LOCAL:
            force_cloud = override in {RouteOverride.CLOUD_AUTO, RouteOverride.FORCE_CLOUD_MODEL}
            result = self._execute_diagnostic(
                diagnostic_request,
                current_trace,
                force_cloud=force_cloud,
                forced_model=forced_model if override is RouteOverride.FORCE_CLOUD_MODEL else None,
            ...
            decision = RouteDecision(result.routing_path, reasons, features, override.value if administrator else None, not result.diagnostic.cloud_used if result.diagnostic else True)
        elif route.matched and features["open_ended_causal_request"] and override is RouteOverride.AUTO:
            ...
            result = self._unsupported_response(text, normalized, route, "missing_required_argument")
            decision = RouteDecision("clarification", ("missing_required_argument",), features, override.value if administrator else None)
        elif override in {RouteOverride.DETERMINISTIC_LOCAL, RouteOverride.CLOUD_DISABLED}:
            code = "cloud_disabled" if override is RouteOverride.CLOUD_DISABLED else "unsupported_intent"
            result = self._unsupported_response(text, normalized, route, code)
            decision = RouteDecision("unsupported", (code,), features, override.value, True)
            ...
            result = self._unsupported_response(text, normalized, route, "cloud_disabled")
            decision = RouteDecision("unsupported", ("open_ended_reasoning_required", "cloud_disabled"), features, None, True)
    ...
    def synthesize_trace_response(
        ...
        *,
        ...
        if message is None:
            raise SpeechProviderError("trace_denied", "response trace has no assistant message")
        ...
        *,
        ...
        with self._paid_operation_gate:
            if not self.ledger.permits(estimate):
                raise SpeechProviderError("budget_denied", "paid TTS was blocked by the configured budget")
            try:
            ...
            if result.estimated_cost_usd is None:
                raise SpeechProviderError("pricing_unknown", "paid TTS pricing is unavailable")
            self.ledger.record_external(
                provider=result.provider,
                operation_category="tts",
                model=result.model,
    ...
    def transcribe_cloud_preview(
        ...
        *,
        ...
        with self._paid_operation_gate:
            if not self.ledger.permits(estimate):
                raise SpeechProviderError("budget_denied", "paid STT was blocked by the configured budget")
            try:
                ...
                raise
            if result.estimated_cost_usd is None:
                raise SpeechProviderError("pricing_unknown", "paid STT pricing is unavailable")
            self.ledger.record_external(
                provider=result.provider,
                operation_category="stt",
                model=result.model,
    ...
    def credential_status(self) -> dict[str, object]:
        return {
            ...
            "paid_text_enabled": self.settings.cloud.enabled and self.settings.cloud.allow_paid_calls,
    ...
    def _execute_diagnostic(
        ...
        *,
        ...
        if engine is None:
            route = RoutedIntent("unsupported", request.text, message="Local diagnostics are disabled.")
            return self._unsupported_response(request.text, request.text, route, "diagnostics_disabled")
        if force_cloud:
            request = replace(
                request,
    ...
    def _general_cloud_locked(
        ...
        *,
        ...
        if not self.ledger.permits(estimate):
            trace.emit(TraceStage.MODEL, "blocked", reason_code="budget_denied", fields={"model": model, "estimated_ceiling_usd": estimate})
            return self._cloud_failure(trace, normalized, "budget_denied", route)
        trace.emit(
            TraceStage.MODEL,
            "started",
            reason_code=reason_code,
            fields={"provider": "openai", "model": model, "reasoning_effort": effort, "max_output_tokens": max_output_tokens, "tools_exposed": [item["name"] for item in tools]},
        )
        ...
        for round_index in range(request_limit):
            if time.perf_counter() - cloud_started >= self.settings.cloud.max_wall_seconds:
            ...
            try:
                ...
                configuration = ReasoningConfiguration(EscalationLevel.ANALYSIS, model, effort)
                self.ledger.record(
                    "general",
                    configuration,
                    CloudTokenUsage(),
            ...
            if turn.response_text is not None:
                return ServiceResponse(
                    ...
                    model=model,
                    reasoning_effort=effort,
                    usage={**asdict(total_usage), "estimated_cost_usd": round(total_cost, 8), "tool_calls": tool_calls},
                    stopping_reason=turn.stopping_reason,
                )
            if turn.tool_request is None:
                return self._cloud_failure(trace, normalized, "cloud_no_conclusion", route)
            tool_calls += 1
            ...
            trace.emit(
                ...
                fields={"skill": request.name, "arguments": request.arguments, "action_class": "read_only"},
            )
            ...
            trace.emit(
                TraceStage.TOOL,
                "complete",
                fields={"skill": request.name, "latency_ms": round(execution.elapsed_seconds * 1000, 3), "result_summary": safe_result},
            )
            previous = turn.response_id
            tool_output = {
                "type": "function_call_output",
                "call_id": request.call_id,
                "output": json.dumps(safe_result, separators=(",", ":"), ensure_ascii=True),
            }
        return self._cloud_failure(trace, normalized, "cloud_request_limit", route)

    def _cloud_failure(self, trace: ExecutionTrace, normalized: str, code: str, route: RoutedIntent) -> ServiceResponse:
    ...
    def _from_assistant_result(
        ...
        trace.emit(
            TraceStage.RESPONSE,
            "ready",
            fields={"response_text": result.response_text, "route": decision.selected_route, "model_avoided": decision.model_avoided},
        )
        return ServiceResponse(
            trace.trace_id,
            trace.request_id,
            ...
            model=model,
            reasoning_effort=effort,
            usage={"estimated_cost_usd": result.diagnostic.estimated_cost_usd, "tool_calls": result.diagnostic.tool_calls} if result.diagnostic else None,
    ...
    def _finish(
        ...
        self.sessions.add_message(session, "assistant", response.response_text, trace.trace_id)
        if not any(event.stage == TraceStage.RESPONSE.value for event in trace.events):
            trace.emit(TraceStage.RESPONSE, "ready", fields={"response_text": response.response_text, "route": response.route})
    ...
    def _diagnostic_request(self, text: str, override: RouteOverride) -> DiagnosticRequest | None:
        planner = self.assistant.diagnostic_planner
        if planner is None:
            return None
        cloud_allowed = override not in {RouteOverride.CLOUD_DISABLED, RouteOverride.DETERMINISTIC_LOCAL, RouteOverride.LOCAL_DIAGNOSTIC}
    ...
    def _relevant_skill_tools(
        ...
        if any(word in text for word in ("server", "pi", "memory", "load", "disk", "service")):
            names.update({"get_server_health", "get_host_observation", "get_stack_observation"})
        if any(word in text for word in ("grafana", "mqtt", "influx", "dashboard", "tailscale")):
        ...
        for name in sorted(names):
            if name in catalog:
                ...
                tools.append({"type": "function", "name": name, "description": definition.description, "parameters": definition.parameters, "strict": True})
                continue
            schema = self._promoted_schema(name)
            spec = self.assistant.skills.get(name)
            if spec is not None and schema is not None:
                tools.append({"type": "function", "name": name, "description": spec.description, "parameters": schema, "strict": True})
    ...
    def _prefetch_local_evidence(
        ...
        for name, arguments in candidates[:3]:
            ...
            trace.emit(
                ...
                reason_code=execution.failure.code if execution.failure else "local_evidence_prefetch",
                fields={
                    "skill": name,
                    "arguments": arguments,
                    "latency_ms": round(execution.elapsed_seconds * 1000, 3),
    ...
    def _promoted_schema(self, name: str) -> dict[str, object] | None:
        entity_ids = [item.entity_id for item in self.assistant.router.entities.entities if item.sensor_type != "printer"]
        values: dict[str, tuple[str, list[str]]] = {
            "get_host_observation": ("metric", ["uptime", "load", "memory", "swap", "disk", "temperature", "throttle", "failed_units"]),
            "get_stack_observation": ("component", ["mqtt", "bridge", "dashboard", "influxdb", "grafana", "home_assistant", "services"]),
            "get_network_observation": ("view", ["interfaces", "routes", "tailscale", "listeners"]),
        }
        if name == "get_sensor_history_summary":
            return _object_schema(
                {"entity": {"type": "string", "enum": entity_ids}, "range_key": {"type": "string", "enum": ["1h", "24h", "7d"]}},
                ["entity", "range_key"],
            )
