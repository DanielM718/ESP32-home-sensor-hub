...
# Registered actions this planner slice may NAME. This is a proposal catalog,
# not the model tool catalog in butters.llm.catalog: nothing here becomes
...
# Per-action parameter allow-lists narrow a registered schema further for the
# planner only. Empty until an action with a free-ish enum is exposed.
CONVERSATIONAL_PLANNER_PARAMETER_ENUMS: dict[str, dict[str, tuple[str, ...]]] = {}
# The reviewed subset that must be frozen in `pending_confirmation` so the
# operator confirms that exact plan before authentication. Membership is a
# reviewed decision, not a derived one: SkillSpec.confirmation_required is True
# for every action built by the shared action() helper, so it cannot
# discriminate here.
PLANNER_CONFIRM_ACTIONS = frozenset({"shutdown_desktop"})
# Ordered multi-step sequences this slice is allowed to freeze. A physical
# multi-stage sequence must be a registered workflow action or an explicitly
# reviewed composition template; the planner may not assemble one itself just
# because the step count is small. Empty here, so this slice freezes exactly
# one action per plan.
PLANNER_COMPOSITIONS: frozenset[tuple[str, ...]] = frozenset()
...
class BetaAssistantService:
    def __init__(
        ...
        *,
        ...
        self.planner_validator = PlannerValidator(
            ...
            max_actions=settings.planner.max_actions,
            compositions=PLANNER_COMPOSITIONS,
        )
    ...
    def plan_conversation(
        ...
        # Two independent gates. Configuration alone must be able to switch
        # planning off: without this, `[planner].enabled = false` would be a
        # flag that reads as a control and enforces nothing.
        if not self.settings.planner.enabled or not self.planner_provider.available:
            self._audit_planner(
                session,
                request=cleaned,
                raw_plan=None,
        ...
        # plan.requires_confirmation is the registry floor, which is True for
        # every action the shared helper builds. What the operator actually sees
        # is the reviewed set below, so report that and never overstate it.
        plan_dict["requires_confirmation"] = any(
            step.action_id in PLANNER_CONFIRM_ACTIONS for step in plan.steps
        )
        self._audit_planner(
            session,
            request=cleaned,
